...
| D15 | **入向验签 userId 维持平台固定值——「选项 B」否决留档（2026-09-01 审计裁决）**：曾提案将 D14「ZA userId = `x-wop-appkey`」扩展至入向 verify（平台响应/回调验签）；裁决**否决**。理由：① D14 权威边界仅覆盖出向 `buildRequest`（条文明言「ZA 与出向请求必须使用同一值」，2026-08-31 飞书裁决），扩展入向属新立法而非既有裁决适用；② 入向报文签名身份属**平台域**——平台加签响应/回调 ZA 使用协议固定值 `1234567812345678`（平台侧签名、interop 样本平台签名、Java 网关侧参考实现（wop-java-sdk，`1234567812345678`）三方同源），SDK 若钉死 appKey 将与平台加签行为不一致、入向验签必然失败；③ 已合入 main 的六仓入向实现一致（interop CI 双绿，2026-09-01 M2 取证）；.NET D14 整改过程中曾将入向 userId 切至 `config.appKey`（与 Go/Java 平台固定值分叉、该路径无绿色正向覆盖）——分叉芽恰证入向语义需本裁决定锚防漂移。**维持现状**：入向 verify 的 ZA userId = 平台协议固定值（已合入 main 的参考实现锚点：wop-go-sdk `sm2PlatformUserID`（signature.go）、wop-java-sdk `Sm2Support.DEFAULT_USER_ID`（Sm2Support.java:23，同值）；Java 落地 PR #26（评审中）拟将其演进为私有 `PLATFORM_SIGN_USER_ID`）；出向 D14 原条款不变。平台侧若未来协议化定义响应签名身份语义，另立新条款 | **已裁决**（2026-09-01） |
