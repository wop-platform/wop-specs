...
> 背景：wop-sdk-config-spec（各语言 SDK 配置与客户端规范，同仓 `docs/specs/wop-sdk-config-spec.md`）评审提出三项上游对齐事项（U1–U3），经修订 PR 裁决落地。三项均为描述性勘误、能力扩张与范围声明，零协议行为变更。本附录条款与正文同级生效；§1.1 适配器表与 §2 概念 API 注已同步修正。
